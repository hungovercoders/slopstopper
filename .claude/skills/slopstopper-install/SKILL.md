...
- Composite actions under `.github/actions/ss-*/` (`ss-setup`, `ss-resolve-url`) — the steps every workflow shares. Refreshed wholesale on every run, never tracked in the marker, never dropped by a profile; a hand edit there is overwritten next refresh, so customise the workflow instead
