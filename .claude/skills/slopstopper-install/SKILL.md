...
| Shape | Profile | Install with |
| ----- | ------- | ------------ |
| Serves HTML to a browser | `ui` (default) | `bash install.sh` |
| JSON/gRPC API, no browser surface | `api` | `bash install.sh --profile api` |
| Library, CLI or package, nothing deployed | `library` | `bash install.sh --profile library` |
...
**Under `--profile library` there is no Pass B** — every URL-driven check is dropped, so Pass A *is* the local loop. Under `--profile api` only DAST survives, so run that one and skip the rest. `slopstopper profile show` tells you which of the commands below still have a workflow behind them; running a dropped check locally still works (the registry is complete regardless of profile), it just isn't gating anything in CI.
...
- A profile is added, or a profile's `disables` list changes in `cli/slopstopper/data/profiles.json` → update the shape table in Step 1's pre-flight framing, the per-profile workflow counts (Step 1.2, Step 3), the Pass B callout in Step 7, and the `profile:` bullet in the Refresh-only knobs list.
