...
5. **How does the target manage security headers?** The CSP-exceptions drift check (`ss-hygiene-csp-exceptions-check.yml`) reads from `app/_headers` or `public/_headers` (Cloudflare native convention; same format on Netlify). If neither exists, the check skips gracefully. Sites that manage headers via framework middleware or platform config (Vercel `vercel.json`, etc.) won't trigger the check — fine on its own, but you'll want a `_headers` file for the security headers themselves (DAST relies on them).
