...
|---|---|---|---|---|
| `Node.js vXX is not supported by Astro!` on `npm run build` across every reliability/Playwright workflow | Check `package.json` `engines.node` vs the `node-version: '20'` in `.github/workflows/ss-*.yml` `actions/setup-node` steps | Workflows pin Node 20; target needs newer | Bulk-edit `node-version:` in each affected workflow | code (config) |
...
| `task ss:hygiene:csp-exceptions` reports "no headers file to check, skipping" with exit 0 | The target doesn't have `app/_headers` (or `public/_headers`) — the check has nothing to guard | Expected and harmless. If you do manage CSP elsewhere (framework middleware, platform config) and want the drift check active, point `paths.headers` in `.slopstopper.yml` at your source file | n/a (graceful skip) |
...
| `task ss:reliability:smoke` fails: `expected /og-image.png to return 200` or wrong CORP header | `curl -I http://localhost:8080/og-image.png` | Site has no site-wide og-image, or local server isn't applying prod headers | Add a 1200×630 `public/og-image.png`; add `Cross-Origin-Resource-Policy: cross-origin` for that path; ensure the `server.js` shim parses the platform's headers file (`public/_headers` on Cloudflare) | code |
...
| `ss-hygiene-auto-label-pr.yml` fails with `The config file was not found at .github/labeler.yml` | `ls .github/labeler.yml` | Slopstopper ships the workflow but not the config — labels are repo-specific | Ship a `.github/labeler.yml` mapping the target's directory globs to labels (`actions/labeler@v5` format) | code (config) |
...
- **`ss-hygiene-csp-exceptions-check.yml`** — guards drift between `app/_headers` (or `public/_headers`) and `docs/security/CSP_EXCEPTIONS.md`. If the target doesn't manage CSP via a `_headers` file, the check skips gracefully — but you can still delete the workflow if you want zero noise in CI.
