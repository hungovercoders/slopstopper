...
| `task ss:hygiene:csp-exceptions` fails with `worker/headers.json not found` | Does the target use a `worker/headers.json` file for CSP? Most don't | Check is slopstopper.dev-specific; target manages headers via adapter / middleware / platform config | Delete `ss-hygiene-csp-exceptions-check.yml` — the check has nothing to guard. `.ss/.workflows-installed` remembers the deletion | delete check |
