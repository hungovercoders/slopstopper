...
| `task ss:hygiene:csp-exceptions` reports "Unknown headers.format" | Run `python3 -c "import sys; sys.path.insert(0,'.ss/scripts'); import headers_adapters; print(list(headers_adapters.ADAPTERS.keys()))"` | `.slopstopper.yml` `headers.format` doesn't match a shipped adapter | Set `headers.format` to one of the listed adapter names or `auto`. If you need a format slopstopper doesn't ship, add a module under `.ss/scripts/headers_adapters/` and register it in `__init__.py`'s `ADAPTERS`. | config (or new adapter) |
