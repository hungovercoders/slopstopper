...
- **`ss-reliability-api-health-check.yml` + `ss-security-api-headers-check.yml`** — only if the repo serves no API at all. They're already inert unconfigured, so deleting them is tidiness rather than necessity; `profile: library` drops both as part of a coherent set, which is the better lever.
