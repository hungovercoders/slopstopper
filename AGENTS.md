...
>
> Underneath, **mise** owns tool *versions* (the `slopstopper-cli` + `task` pins
> in `mise.toml`, activated per-directory) while Task owns *commands* — the
> idiomatic mise+Task split. mise installs the pinned CLI locally and in CI
> (`jdx/mise-action`); see [`docs/architecture/README.md`](./docs/architecture/README.md).
...
| Editing any skill under `.claude/skills/slopstopper-*/` (the map or its `references/`) | [`install-skill.sh`](./install-skill.sh) (validates fetched file has frontmatter; if you add or rename a skill, also update its `SKILLS` array AND `install.sh`'s `SKILL_NAMES` array), [`docs/runbooks/INSTALL_SKILLS.md`](./docs/runbooks/INSTALL_SKILLS.md), `README.md` "Using Claude Code?" section, `app/index.html` "Claude Code users" section, and the other skill's "When to hand off" pointer |
